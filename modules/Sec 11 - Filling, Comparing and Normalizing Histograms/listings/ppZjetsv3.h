...
// Header file to include declarations, classes, variables for ppZjets.C macro
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ppZjets.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ppZjets.root");
      }
...
Int_t ppZjets::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ppZjets::LoadTree(Long64_t entry)
{
...
Int_t ppZjets::Cut(Long64_t entry)
{
