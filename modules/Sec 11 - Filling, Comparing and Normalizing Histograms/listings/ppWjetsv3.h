...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ppWjets.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ppWjets.root");
      }
...
Int_t ppWjets::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ppWjets::LoadTree(Long64_t entry)
{
...
Int_t ppWjets::Cut(Long64_t entry)
{
