...
To submit jobs to DICE using Soolin, I'll still need to be in the \textbf{\$CMSSW\_\-BASE/src/CMGTools/RA1/cfg/} directory. Then type
...
\lstinputlisting[language=sh, caption={A shell script to download and install AlphaTools with Dom's branch needed to generate the cut flow tables for the 2015 analysis. File name: load\_doms\_alphatools\_setup.sh.}]{listings/load_doms_alphatools_setup.sh}
...
\begin{tabular}{lcccccc}
  \hline
  Event selection & \multicolumn{6}{c}{Benchmark model ($\mSUSY,\,\mLSP$)} \\
  \cline{2-7}
...
\begin{tabular}{lcccc}
  ...
  $\abs{\etajone} < 2.5$ & \phantom{1}25\phantom{.1} & \phantom{1}21\phantom{.1} & \phantom{1}46\phantom{.1} & \phantom{1}24\phantom{.1} \\
...
\begin{tabular}{lccc}
  \hline
  Event selection & \multicolumn{3}{c}{Benchmark model ($\mSUSY,\,\mLSP$)} \\
  \cline{2-4}
  ...
  $\abs{\etajone} < 2.5$ & \phantom{1}15\phantom{.1} & \phantom{1}13\phantom{.1} & \phantom{1}18\phantom{.1} \\
