...
\subsubsection{Cut flow table for benchmarks}
...
\begin{tabular}{lc}
  ...
  $H_{\mathrm{T}}^{\mathrm{miss}} > 200\,\mathrm{GeV}$  & \phantom{1}26\phantom{.1}  \\
  Event veto for forward jets ($|\eta| > 2.4$) & \phantom{1}22\phantom{.1} \\
  $H_{\mathrm{T}}^{\mathrm{miss}} / E_{\mathrm{T}}^{\mathrm{miss}} < 1.25$ & \phantom{1}20\phantom{.1} \\
  $H_{\mathrm{T}}$-dependent $\alpha_{\mathrm{T}}$ requirements ($H_{\mathrm{T}} < 900\,\mathrm{GeV}$)  & \phantom{1}11\phantom{.1} \\
...
\subsection{Analysing the chargino-mediated model (T2bW\_X05, 03/04/2018)}
