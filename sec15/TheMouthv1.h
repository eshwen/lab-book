...
// Wed Nov 23 15:34:07 2016 by ROOT version 5.34/36
// from TTree Delphes/Analysis tree
// found on file: TheMouth.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   const Int_t kMaxParticle = 1000;
   const Int_t kMaxTrack = 142;
   const Int_t kMaxTower = 210;
   const Int_t kMaxEFlowTrack = 142;
   const Int_t kMaxEFlowPhoton = 116;
   const Int_t kMaxEFlowNeutralHadron = 98;
...
#ifdef TheMouth_cxx
TheMouth::TheMouth(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(MouthDirectory);
      if (!f || !f->IsOpen()) {
...
Int_t TheMouth::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TheMouth::LoadTree(Long64_t entry)
{
...
void TheMouth::Show(Long64_t entry)
{
...
Int_t TheMouth::Cut(Long64_t entry)
{
